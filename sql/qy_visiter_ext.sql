with t1 as (select a.BesucherID,b.Typ,YEAR(b.Datum) as year,MONTH(b.Datum) as month,DAY(b.Datum) as day
    ...
    select IFNULL(tVeranstTyp.Bezeichnung,'--') as Bezeichnung,year as Jahr,month as Monat,count(BesucherID) as Besucher from t1 
        left join tVeranstTyp on tVeranstTyp.id=t1.Typ
