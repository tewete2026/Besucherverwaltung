...
def ax_get_theme_overview():
    ...
    if overview_search is not None and len(overview_search) > 0 and overview_search != "ALL":
        if overview_search.isnumeric():
            ...
            sql_parms = f"WHERE a.Thema like {search_like}"
    ...
    try:
        ...
        cur.execute(f"SELECT a.id,a.Thema as bezeichnung from tThemen a \
                    {sql_parms} \
                    ORDER BY a.Thema LIMIT {overview_offset}, {overview_readlines}")
        dbdata.update({"themes":cur.fetchall()})
        len_vis = len(dbdata["themes"])
        show_lines = len_vis
        if len_vis > overview_maxlines:
            show_lines = overview_maxlines
            is_more_lines = True
            
        rc_code["html"] = render_template("verwThemen_body.html", themes=dbdata["themes"][0:show_lines])
